...
const LIMITATION_KEYWORDS = [
  /\bmax(imum)?\b/i, /\blimit(ed|s)?\b/i, /\brate.?limit\b/i,
  /\bonly\b/i, /\bcannot\b/i, /\bcan't\b/i, /\bwon't\b/i, /\bdoes\s+not\b/i,
  /\bup\s+to\b/i, /\bat\s+most\b/i, /\brequires?\b/i, /\bmust\b/i,
  /\bnote:?\b/i, /\bcaveat\b/i, /\bwarning\b/i, /\brestrict/i,
];
...
export const descHasExamples: LintRule = {
  ...
  check(tools) {
    ...
    for (const tool of tools) {
      ...
      const paramDescs = props ? Object.values(props).map((p) => (p as { description?: string }).description ?? '') : [];
      const paramHasExample = paramDescs.some((d) => /\b(e\.?g\.?|example|for\s+instance)/i.test(d));
