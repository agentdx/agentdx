...
function main() {
  ...
  for (const file of files) {
    ...
    writeFileSync(resolve(EXAMPLES_DIR, `${exampleName}-lint.txt`), stripAnsi(output), 'utf-8');
  ...
  console.log(`${'='.repeat(60)}\n`);
  console.log(
    `  ${'Server'.padEnd(40)} ${'Score'.padEnd(8)} ${'E'.padEnd(4)} ${'W'.padEnd(4)} ${'I'.padEnd(4)} Tools`,
  );
  console.log(
    `  ${'─'.repeat(40)} ${'─'.repeat(5)}  ${'─'.repeat(3)} ${'─'.repeat(3)} ${'─'.repeat(3)} ${'─'.repeat(5)}`,
  );
